    ...
    if (!(user.id in Globals.users))
    {
        ...
        timer.on('tick', async () =>
        {
            try
            {
                ...
                debugInfo(`${foundUser.name} POSTED [${timer.timeLeft.asSeconds()}] [${postUrl}]`)
        ...
        timer.on('start', () =>
        {
            debugVerbose(`${user.name} START [${timer.timeLeft.asSeconds()}]`)
        })

        timer.on('stop', () =>
        {
            debugVerbose(`${user.name} PAUSE [${timer.timeLeft.asSeconds()}]`)
        })
